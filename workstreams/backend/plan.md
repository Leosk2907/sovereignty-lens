...
8. Implement the transactional company-profile SQL function. In the same
   transaction, insert the European company, connect one-to-three European
   customers, upsert one-to-three providers, create every edge, write one
   canonical `dependency.created` event per edge to `graph_events`, and
   `pg_notify` the SSE bridge.
...
- A committed profile produces exactly one versioned `dependency.created`
  event per edge, with identifiers matching the API response and stored rows.
...
- **Replaces "Preview deployment returns the seeded graph and accepts a valid
  ...
  seeded graph, accepts a valid company profile, and delivers every resulting
